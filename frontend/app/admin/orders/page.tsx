...
export default function AdminOrdersPage() {
  ...
  const inputCls = "rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:border-rosegold";
    ...
    <div>
        ...
        action={
          <button
            onClick={() =>
              exportCsv(
                "wabil-orders.csv",
                filtered.map((o) => ({ id: o.id, customer: o.customer, email: o.email, date: o.date, status: o.status, total_pkr: o.totalPkr, items: o.items })),
              )
            }
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3 rounded-2xl border border-border bg-card p-4 shadow-luxe">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search order # or customer…" className={`${inputCls} w-full pl-9`} />
        </div>
      ...
      <Drawer open={!!viewing} title={viewing ? `Order #${viewing.id}` : ""} onClose={() => setViewing(null)}>
        {viewing && (
                ...
                {viewing.timeline.map((t, i) => (
                  <div key={i} className="flex gap-3">
                    <div className="flex flex-col items-center">
                      <div className={`flex h-7 w-7 items-center justify-center rounded-full ${t.done ? "bg-rosegold text-white" : "border border-border text-foreground/30"}`}>
                        <Check size={14} />
                      </div>
                      {i < viewing.timeline.length - 1 && <div className={`w-px flex-1 ${t.done ? "bg-rosegold" : "bg-border"}`} style={{ minHeight: 28 }} />}
                    </div>
                    <div className="pb-5">
                      <p className={`text-sm font-medium ${t.done ? "" : "text-foreground/40"}`}>{t.label}</p>
