...
export default function AdminReviewsPage() {
  ...
  const sendReply = (id: string) => {
    setReviews((list) => list.map((r) => (r.id === id ? { ...r, reply: replyText } : r)));
    setReplyFor(null);
    setReplyText("");
    ...
    <div>
      ...
      <div className="mb-5 flex gap-1 border-b border-border">
        {TABS.map((t) => (
          ...
          >
            {t} ({counts[t] ?? 0})
            {tab === t && <motion.div layoutId="review-tab" className="absolute inset-x-2 -bottom-px h-0.5 rounded-full bg-rosegold" />}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-4">
            ...
            >
              ...
              {replyFor === r.id ? (
                <div className="mt-3 flex gap-2">
                  ...
                  />
                  <button onClick={() => sendReply(r.id)} className="rounded-xl bg-charcoal px-4 text-sm font-medium text-ivory">
                    Send
                  </button>
                </div>
              ) : (
                <div className="mt-4 flex gap-2">
                  {r.status !== "approved" && (
                    <button onClick={() => setStatus(r.id, "approved")} className="flex items-center gap-1.5 rounded-lg bg-emerald-600/10 px-3 py-1.5 text-sm text-emerald-600 hover:bg-emerald-600/20">
                      <Check size={14} /> Approve
                    </button>
                  )}
                  {r.status !== "rejected" && (
                    <button onClick={() => setStatus(r.id, "rejected")} className="flex items-center gap-1.5 rounded-lg bg-rose-500/10 px-3 py-1.5 text-sm text-rose-500 hover:bg-rose-500/20">
                      <X size={14} /> Reject
                    </button>
                  )}
                  <button onClick={() => { setReplyFor(r.id); setReplyText(r.reply ?? ""); }} className="flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-sm hover:border-rosegold">
        ...
        {filtered.length === 0 && <p className="py-12 text-center text-foreground/40">No {tab} reviews.</p>}
      </div>
    </div>
  );
}
