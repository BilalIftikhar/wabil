...
export default function AdminCustomersPage() {
  ...
  const toggleBlock = (id: string) => {
    setCustomers((list) => list.map((c) => (c.id === id ? { ...c, blocked: !c.blocked } : c)));
    ...
    <div>
      ...
      <div className="mb-4 rounded-2xl border border-border bg-card p-4 shadow-luxe">
        <div className="relative">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search customers…" className="w-full rounded-xl border border-border bg-background py-2 pl-9 pr-3 text-sm outline-none focus:border-rosegold" />
        </div>
      </div>
      ...
      <div className="overflow-x-auto rounded-2xl border border-border bg-card shadow-luxe">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((c, i) => (
              <motion.tr key={c.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.04, 0.3) }} className="border-b border-border/60 hover:bg-muted/40">
                <td className="p-3">
                ...
                <td className="p-3">
                  <StatusBadge label={c.blocked ? "blocked" : "active"} color={c.blocked ? "#D62828" : "#A3B18A"} />
                </td>
